...
from day06 import is_obstacle, is_out_of_bounds, locate_guard, move_guard, patrol
...
def test_move_guard(data: list[list[str]]):
    ...
    assert data[guard[1]][guard[0]] == "^"
    assert data[orig_y][orig_x] == "0"


def test_move_guar_turn_right(data: list[list[str]]):
    x, y = locate_guard(data)
    data[y][x] = "."
    x, y = 4, 1
